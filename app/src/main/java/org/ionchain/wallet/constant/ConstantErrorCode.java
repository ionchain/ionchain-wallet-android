package org.ionchain.wallet.constant;

/**
 * 错误码,从1000开始
 */
public class ConstantErrorCode {
    //检查更新的错误码
    public static final String ERROR_CODE_NET_WORK_UPDATE = "1001";//检查更新网络错误
    //设别相关的错误码
    public static final String ERROR_CODE_NET_WORK_ERROR_DEVICE_BIND = "1010";//设备绑定网络错误
    public static final String ERROR_CODE_NET_WORK_ERROR_DEVICE_UNBIND = "1011";//设备解除绑定网络错误
    public static final String ERROR_CODE_NET_WORK_ERROR_DEVICE_BIND_NULL = "1012";//设备绑数据解析错误
    public static final String ERROR_CODE_NET_WORK_ERROR_DEVICE_UNBIND_NULL = "1013";//设备解除绑数据解析错误

    public static final String ERROR_CODE_NET_WORK_ERROR_TXRECORD = "1020";//交易记录请求失败
}
